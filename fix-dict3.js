...
function injectRegex(content, locale, dictData) {
    // 1. Find the block for the locale
    // e.g. ar: {
...
transContent = injectRegex(transContent, 'tr', trDict);
transContent = injectRegex(transContent, 'ar', arDict);
transContent = injectRegex(transContent, 'fr', frDict);
transContent = injectRegex(transContent, 'ru', ruDict);

fs.writeFileSync(transFile, transContent);
console.log('Translations dictionary fixed properly (Regex method)!');
