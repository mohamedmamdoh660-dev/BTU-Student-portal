...
//     },
//     register: {
// This was my injection.
// I can remove my injection by finding `    dashboard: {\n      dashboardTitle: `
// and deleting the entire block until `    },\n    register: {`

const injectedRegex = /    dashboard: \{\n      dashboardTitle:(?:[\s\S]*?)    \},\n/g;
content = content.replace(injectedRegex, '');

// Now the file is back to normal.
// We have an existing `    dashboard: {` block somewhere.
// Wait, the existing dashboard block starts with `    dashboard: {`.
// Let's split by `    dashboard: {` and for parts 1..5, we prepend our keys.

const parts = content.split('    dashboard: {');
...
if (parts.length === 6) {
    ...
    for (let i = 1; i <= 5; i++) {
        ...
        newContent += injectStr + parts[i].trimStart(); // The trimStart() will eat `\n` but we added `\n` in injectStr.
    }
