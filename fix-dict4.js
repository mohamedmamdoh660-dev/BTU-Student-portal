...
function applyDict(loc, d) {
    ...
    const hasKeys = content.includes(`      haveTc:`);
    if(loc === 'tr' && content.includes(`T.C. Vatandaşı mısınız`)) return;
    if(loc === 'ar' && content.includes(`هل لديك جنسية تركية`)) return;
    
    for (let i = 0; i < lines.length; i++) {
        const line = lines[i];
        if (!inLoc && line.match(new RegExp(`^\\s*${loc}:\\s*{`))) {
            inLoc = true;
        }
        
        output.push(line);
        
        if (inLoc && !foundRegister && line.match(/^\s*register:\s*{/)) {
            foundRegister = true;
            
            // Inject here
            for (let k in d) {
                // If it doesn't already exist on the next lines, insert
                // But we know it doesn't exist. So just inject!
                output.push(`      ${k}: "${d[k].replace(/"/g, '\\"')}",`);
            }
        }
        
        // We stop matching locale block when we see exactly `  },`
        if (inLoc && line.match(/^  },?$/)) {
            inLoc = false;
            foundRegister = false;
        }
...
fs.writeFileSync(transFile, lines.join('\n'));
console.log('Done!');
